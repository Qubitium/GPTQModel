...
class SwordfishLinear(GPTQQuantLinear):
    ...
    def post_init(self):
        ...
        if self.sym:
            ...
            # AWQ-style zero points: fold (8 - zp) * scale into fp group tensor.
            ...
            qzeros_fp = ((8.0 - qzeros_unpacked.to(scales.dtype)) * scales).contiguous()
            replace_parameter(
                self,
                "qzeros",
        ...
        if perm is not None:
            self.register_buffer("input_perm", perm.to(device=device))
        else:
            self.register_buffer("input_perm", torch.empty(0, dtype=torch.int32, device=device))
