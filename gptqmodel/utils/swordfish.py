...
def _swordfish_extra_cuda_cflags() -> List[str]:
    ...
    # plausibly build them (CUDA 13+). When cross-compiling, set
    # TORCH_CUDA_ARCH_LIST explicitly.
    arch_flags: Tuple[str, ...] = _SWORDFISH_ARCH_FLAGS
    if any("100" in flag for flag in resolved_cuda_arch_flags()):
        # Let the user override with a more specific arch list and avoid
        # duplicating the default sm_103 flags that PyTorch would also emit.
        pass
