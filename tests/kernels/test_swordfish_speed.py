...
def _copy_to_swordfish(torch_linear: TorchLinear) -> SwordfishLinear:
    ...
    sf.scales = nn.Parameter(torch_linear.scales.data.detach().clone().to(torch.float16).contiguous(), requires_grad=False)
...
class TestSwordfishSpeed(unittest.TestCase):
    ...
    def test_swordfish_speed_vs_machete(self):
        ...
        for m in m_values:
            ...
            try:
                ...
                if machete_runtime_available():
                    ...
                    mach.scales = nn.Parameter(torch_linear.scales.data.detach().clone().to(torch.float16).contiguous(), requires_grad=False)
                    mach.post_init()
                    mach = mach.to(device)
        ...
        if 1 in swordfish and 1 in baseline:
            self.assertGreater(swordfish[1], baseline[1] * 0.5,
                               "Swordfish bs=1 throughput should be within 2x of dense GEMM")
